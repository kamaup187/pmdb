...
class ClientInvoice(Resource):
    ...
    def get(self):
        ...
        # diff = timenow.day - 2
        # invdate = bill.date - relativedelta(days = diff)
        invdate = generate_exact_date(2,3,2026)
        inv_date = invdate.strftime("%d/%b/%y")
...
class SendInvoices(Resource):
    ...
    def post(self):
        ...
        if sms_units:
            ...
            advanta_send_sms(f'Alert: Failed to fetch SMS units balance for {current_user.company.name} for prop {prop}',"+254716674695",kiotapay_api_key,kiotapay_partner_id,"RENTLIB")
...
class ReceivePayment(Resource):
    ...
    def post(self):
        ...
        if tenant_obj.tenant_type == "owner" or tenant_obj.tenant_type == "resident":
            ...
            specific_charge_obj = fetch_target_period_tenant_invoice_update(tenant_obj,house_obj,pay_period_date)
        ...
        existing_trans = AppTransactionOp.fetch_transaction_by_ref(bill_ref)
        if existing_trans:
            pass
        else:
...
class UpdateDeposit(Resource):
    ...
    def get(self):
        ...
        if not dep:
            ...
            print("TENANT DEPOSIT TOTAL MISSING")
            # if not dep.total:
            #     TenantDepositOp.update_active(dep,False)
...
class CallBackUrlAstrolThika(Resource):
    def get(self):
        pass
    def post(self):
        #parse for json
