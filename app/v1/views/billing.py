...
class ClientInvoice(Resource):
    ...
    def get(self):
        ...
        # diff = timenow.day - 2
        # invdate = bill.date - relativedelta(days = diff)
        invdate = generate_exact_date(31,7,2025)
        inv_date = invdate.strftime("%d/%b/%y")
...
class ReceivePayment(Resource):
    ...
    def post(self):
        ...
        trans = AppTransactionOp(bill_ref,pay_date,tenant_obj.name + " (" + narration + ")",paidll_bool,payment_obj.id,house_obj.apartment.name,house_obj.name,bank,valid_amount,"debit",co.id)
        trans.save()
